...
def write_output(filename, algorithm_name, processes, gantt_chart):
    with open(filename, "w", encoding="utf-8") as f:
        ...
        # Dosyaya Yazdirma (Odevin b ve c maddeleri)
...
def run_fcfs(processes):
    ...
    write_output("FCFS_Sonuc_case2.txt", "FCFS", results, chart)

def run_sjf_non_preemptive(processes):
    results, chart = schedule_SJF_NonPreemptive(processes)
    write_output("SJF_NonPreemptive_Sonuc_case2.txt", "Non-Preemptive SJF", results, chart)

def run_priority_non_preemptive(processes):
    results, chart = schedule_Priority_NonPreemptive(processes)
    write_output("Priority_NonPreemptive_Sonuc_case2.txt", "Non-Preemptive Priority", results, chart)

def run_sjf_preemptive(processes):
    results, chart = schedule_SJF_Preemptive(processes)
    write_output("SJF_Preemptive_Sonuc_case2.txt", "Preemptive SJF (SRTF)", results, chart)

def run_round_robin(processes):
    results, chart = schedule_RoundRobin(processes, time_quantum=4)
    write_output("RoundRobin_Sonuc_case2.txt", "Round Robin", results, chart)

def run_priority_preemptive(processes):
    results, chart = schedule_Priority_Preemptive(processes)
    write_output("Priority_Preemptive_Sonuc_case2.txt", "Preemptive Priority", results, chart)

if __name__ == "__main__":
    
    # Kullanıcıdan dosya ismini istiyoruz (input ile)
    dosya_adi = input("Lutfen okunacak dosya adını yazın (Örn: case2.csv): ")

    # Eğer kullanıcı bir şey yazmadan Enter'a basarsa otomatik case2.csv'yi seçer
    if dosya_adi == "":
        dosya_adi = "case2.csv"
    all_processes = load_processes(dosya_adi)
    
    if all_processes:
        print("MULTITHREADING BASLATILIYOR (CASE 2)")
